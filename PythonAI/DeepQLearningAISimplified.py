...
class DeepQLearningAISimplified(AIInterface):
    def __init__(self):
        super().__init__()
...
### Simplified amount of convolutional layers
class DQN(nn.Module):
    ...
    def __init__(self, actions, obsv_width, obsv_height):
        ...
        self.pre_linear_width = round(obsv_width/(2**1))
        self.pre_linear_height = round(obsv_height/(2**1))
        self.fc1 = nn.Linear(32 * self.pre_linear_width * self.pre_linear_height, 512)  # Adjust the input features based on the output of the last pooling layer
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 32 * self.pre_linear_width * self.pre_linear_height)  # Flatten the tensor
